...
class ProviderStreamChunk(BaseModel):
    ...
    @classmethod
    def _coerce_chunk(cls, value: Any) -> Any:
        if isinstance(value, cls):
            return value
        if not isinstance(value, dict):
            return value

        event = value.get("event") or value.get("event_type")
        if event is None:
        ...
        if data_payload is None:
            ignored_keys = {"event", "event_type", "raw", "error"}
            data_payload = {
                key: value[key]
                for key in value
            ...
            async with client.stream(
                ...
                async for raw_line in response.aiter_lines():
                    ...
                    if not data_str:
                        continue
                    if data_str == "[DONE]":
                        yield ProviderStreamChunk(event="done", data={})
                        break
                    try:
                        payload_chunk = json.loads(data_str)
                    except json.JSONDecodeError:
                        continue
                    yield self._normalize_stream_payload(
                        payload_chunk,
                        self.defn.model or model,
                    )
